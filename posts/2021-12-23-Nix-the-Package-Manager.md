...
- [`shell.nix`]() => this files contains every package I want to provide (`stack` to create the website, `ormolu` to format Haskell files, `hlint` to get hint on Haskell, `nixfmt` to format Nix files and [`niv`](https://github.com/nmattia/niv) to update nixpkgs)
- `nix/default.nix` => empty in my case, but useful if you have to provide unfree packages or broken ones
- `nix/sources.json` and `nix/sources/nix` => do not touch these two files. They are generated by `niv`
- `.envrc` to use [`direnv`](https://direnv.net/) to make an implicit `nix-shell` at each `cd` to this folder
